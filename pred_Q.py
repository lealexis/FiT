...
class PredFrameLen(object):
    def __init__(self, max_q=0.8, min_q=0.2, eff_load=None, freq_q=None, 
        ...
        if freq_q is None:
            self._wf_q = np.pi / 5  # default
        else:
            self._wf_q = 2 * np.pi * freq_q
        
        if eff_load is None:
            self._eff_load = 40  # default
        else:
        ...
        # approximate generation time of an EPR pair
        EPR_GEN_SPAN = 1.16

        avg_max_dur = (((2 / np.pi) * amp_len) + bias_len) * EPR_GEN_SPAN
        avg_min_dur = (bias_len - ((2 / np.pi) * amp_len)) * EPR_GEN_SPAN
    ...
    def get_frame_len(self):
        ...
        shift = (self._amp_q_shift * np.sin(self._wf_q * t + self._phi) 
                 + self._bias_q_shift)
        q = (self._amp_q_sin * np.sin(self._wf_q * t + self._phi + shift) 
             + self._bias_q_sin)
        frm_len = self._eff_load / (1 - q)
        frm_len = trunc(frm_len)
        if frm_len <= self._eff_load:  # at least 3 measurements, X,Y and Z.
            frm_len = self._eff_load + 3
        # ensure that amount of qubits to measure are divisible by 3.
        while (frm_len - self._eff_load) % 3 != 0:
            frm_len += 1
        q_pract = 1 - (self._eff_load / frm_len)
        df_frm_len = pd.DataFrame([[t, q, q_pract, frm_len]], 
                                  columns=["time", "q", "q_pract", 
                                           "frame_len"])
