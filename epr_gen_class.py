...
class EPRgenerator(object):
    def __init__(self, host: Host, max_fid=0.99, min_fid=0.5, max_dev=0.1,
        ...
        if max_fid < min_fid:
            raise EPRgenBadParameterException("max_fid must be bigger than"
                                              " min_fid.")
        if (max_fid > FID_UPPER_LIMIT) or (min_fid < FID_LOWER_LIMIT):
        ...
        if (max_dev > MAX_DEV_LIM) or (min_dev < MIN_DEV_LIM):
        ...
        if spread is None:
            self.spread = 0.2  # default std. deviation
    ...
    def _get_fidelity_4_gen(self):
        ...
        if f > FID_UPPER_LIMIT:
            f = FID_UPPER_LIMIT - 0.01
        if f < FID_LOWER_LIMIT:
            f = FID_LOWER_LIMIT
        return f
    ...
    def _fidelity(self, epr_half: Qubit):
        ...
        \ket{phi^{+}}, for an imperfect EPR Pair with state vector as 
        follows
        B1 = a_{1} + ib_{1}
        B2 = a_{2} + ib_{2}
        B3 = a_{3} + ib_{3}
        ...
        if np.shape(epr_ket)[0] == 4:  # a valid epr_ket
            ...
            raise ValueError("Argument passed to epr_halve is not a two"
    ...
    def _gen_epr(self, rx1=0, ry1=0, rx2=0, ry2=0):
        """Generate an EPR-Pair by applying x and y rotational gates 
        with angles rx1, ry1, rx2, and ry2 to the first and second qubit
        before passing them trough the entanglement circuit. If default, 
        rotation angles are used a perfect EPR-Pair is generated."""
        ...
        df_gen = pd.DataFrame([[gen_time, gen_f]], 
                              columns=["time_gen", "fid_gen"])
    ...
    def get_epr_pair(self):
        """ EPR-Pairs are generated by applying X and Y rotation gates
        to the zero ket qubits before entering the entanglement circuit.
        The rotation angles have the same value alpha which generate a
        EPR-Pair with fidelity F, which could drawn from sinusoidal mean
        and standard deviation value for the gaussian distribution(check
        _get_fidelity_4_gen) or from a simple gaussian distribution. """
