...
class EPRgenerator(object):
    ...
    def _gen_epr(self, rx1=0, ry1=0, rx2=0, ry2=0):
        """Generate an EPR-Pair by applying x and y rotational gates with 
        angles rx1, ry1, rx2, and ry2 to the first and second qubit before
        passing them trough the entanglement circuit.
    ...
    def get_epr_pair(self):
        """ EPR-Pairs are generated by applying X and Y rotation gates to the
        zero ket qubits before entering the entanglement circuit. The rotation
        angles have the same value alpha which generate a EPR-Pair with
        fidelity F, which could drawn from sinusoidal mean and standard
        deviation value for the gaussian distribution(check
