...
class EPR_buff_itfc(object):
    ...
    def _append_mssg_to_ID_ip(self, mssg:str):
        if type(mssg) is not str:
            ...
            if self.in_process and (self._get_MSSG_in_process() == 
                                    "EPR:storing"):
    ...
    def store_EPR_PHASE_1(self, epr_half: Qubit):
        ...
        if self.in_process and valid_mssg:
            qids = self._get_qubit_ids_from_fr_ID(id_f=
                                                  self._get_ID_in_process())
        ...
        if (self.in_process and self._get_MSSG_in_process() == 
            ...
            try:
                ...
                print("the qubit id: {id} is corrupted or is not in frame "
                      "{FrID}".format(id=qubit_id, FrID=frame_id))
            else:
    ...
    def set_F_EPR_END_PHASE(self, F_est=None, to_history=False):
        if to_history:
            ...
            if (self.in_process and self._get_MSSG_in_process() == 
                ...
                raise BadPhaseCallException("set_F_est_EPR_END_PHASE can be"
    ...
    def nuID_CORRECT_epr_as_sdc_EPR_PHASE_2(self):
        if self.is_receiver:
            if (self.in_process and (self._get_MSSG_in_process() ==
                ...
                raise BadPhaseCallException("nxt_uID_CORRECT_epr_as_sdc_EPR_"
                                            "PHASE_2 can be called only after"
                                            " EPR_PHASE_1.")
        else:
    ...
    def pop_sync_SDC_END_PHASE(self):
        ...
        SDC-Frame *rcv_ID* is poped alongside its corresponding stored
        EPR-Frame *sto_ID* in order to decode the SDC-encoded classical
        information."""
        ...
        if self.is_receiver:
            ...
            if self.in_process and mssg_is_valid:
                ...
                if (len(ip_IDs) == len(ip_MSSGs) == 2):
                    ...
                    if m1 and m2:
                        if len(rcv_qids) == len(sto_qids) == self.eff_load:
                            self._set_mssg_to_ID_ip(["SDC:sync_rtrv_rec",
                                                     "SDC:sync_rtrv_sto"])
                        else:
                ...
                raise BadPhaseCallException("pop_sync_SDC_END_PHASE can "
                                            ...
                                            "nxt_uID_CORRECT_epr_as_sdc_"
                                            "EPR_PHASE_2 until all stored "
